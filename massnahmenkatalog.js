...
const AUDIENCE_CATALOG = [
    ...
    {
        targetId: 'developerCatalog',
        titleId: 'developerTitle',
        tag: 'Für Hersteller:innen und Entwickler:innen',
        title: 'Maßnahmen in Produktgestaltung, Datenverarbeitung und Kommunikation',
        intro: 'Diese Perspektive bündelt Produktentscheidungen, Kommunikationsmaßnahmen und technische Vorkehrungen, die Privatheit über den gesamten Produktlebenszyklus stärken können.',
        phases: {
            'before-purchase': {
                social: [
                    'Ein Konsens-Protokoll oder vergleichbare Gesprächshilfen für Haushalte bereitstellen.',
                    'Den Datenfluss mit Privacy Label, klaren Erklärungen und verständlichen Beispielen transparent machen.',
                    'Interaktive Privacy Labels oder smarte Profile nutzen, damit Folgen der Datennutzung nachvollziehbar werden.'
                ],
                technical: [
                    'Privacy-by-Default als Ausgangspunkt setzen.',
                    'Bereits vor dem Kauf klar ausweisen, welche Daten erhoben und gespeichert werden.',
                    'Datenminimierung und reduzierte Datenspeicherung als Produktprinzip verankern.',
                    'Cloud- und Hosting-Entscheidungen transparent machen, etwa lokal versus Cloud sowie Speicherorte innerhalb oder außerhalb der EU.'
                ]
            },
            setup: {
                social: [
                    'Rollen und Sichtrechte so erklären, dass Haushalte Verantwortlichkeiten schon beim Einrichten aushandeln können.',
                    'Konfliktträchtige Freigaben, etwa zwischen Vermieter:in und Mieter:in oder in Familienkonstellationen, sichtbar machen.'
                ],
                technical: [
                    'Einen zweiten Admin-Account oder gleichwertige Recovery-Mechanismen ermöglichen.',
                    'Rollen-Rechte-Modelle und getrennte Sichtrechte im Produkt abbilden.',
                    'Schwellwerte für Datenerfassung konfigurierbar machen, etwa Lautstärke erst ab einem bestimmten Wert zu speichern.',
                    'Zeitabhängige oder selbst definierbare Schwellenwerte beim Set-Up anbieten.'
                ]
            },
            use: {
                social: [
                    'Rohdatenzugriffe an Konsens und Stakeholder-Absprachen koppeln.',
                    'Daten verständlich aufbereiten, etwa als Ja/Nein-Aussagen, Extremwerte oder handlungsleitende Hinweise statt schwer lesbarer Rohdaten.',
                    'Nicht nur Zustände anzeigen, sondern alltagsnahe Hinweise geben, zum Beispiel statt nur „zu feucht“ auch „lüfte mal wieder“.'
                ],
                technical: [
                    'Automatisches und manuelles Löschen von Daten ermöglichen.',
                    'Prüfen, ob Historien wirklich nötig sind oder ob Live-Daten, Aggregationen oder kürzere Speicherfristen genügen.',
                    'Zwischen Rohdaten und aggregierten Daten differenzieren und bevorzugt mit datensparsamen Darstellungen arbeiten.',
                    'Systemintelligenz so einsetzen, dass Nutzende interpretierte Ergebnisse sehen, ohne unnötig feingranulare Daten offenzulegen.'
                ]
            },
            disposal: {
                social: [
                    'Zum Produktende klare Hinweise geben, wie Haushalte Zugriffe beenden, Daten entfernen und Verantwortlichkeiten sauber abschließen.',
                    'Dokumentieren, welche Schritte beim Gerätewechsel oder bei der Entsorgung für alle Mitbetroffenen relevant sind.'
                ],
                technical: [
                    'Datenlöschung auch am Lebensende des Produkts zuverlässig unterstützen, lokal wie in der Cloud.',
                    'Export-, Lösch- und Deprovisionierungsprozesse so gestalten, dass keine verwaisten Konten oder Restdaten bleiben.'
                ]
            }
        }
    }
...
const SECURITY_PRACTICES = [
    ...
    {
        title: 'Allgemeine Sicherheitspraktiken für Hersteller:innen und Entwickler:innen',
        intro: 'Sicherheit sollte nicht nur technisch umgesetzt, sondern auch nachvollziehbar kommuniziert und überprüfbar gemacht werden.',
        groups: {
            social: [
                'Sicherheits- und Recovery-Prozesse verständlich dokumentieren.',
                'Verantwortlichkeiten für Adminrechte, Support und kritische Änderungen klar beschreiben.'
            ],
            technical: [
                'Plausibilitätsprüfungen nutzen, um fehlerhafte oder missverständliche Sensordaten früh zu erkennen.',
                'Datenverschlüsselung für gespeicherte Daten umsetzen.',
                'Transportverschlüsselung für jede Kommunikation zwischen Gerät, App und Backend verpflichtend machen.',
                'Sicherheitsrelevante Rechte nach dem Prinzip minimaler Berechtigung vergeben.'
            ]
        }
    }
];
...
function handlePhaseTabKeydown(event) {
...
function initPhaseTabs() {
    $$('.phase-tab').forEach(tab => {
        ...
        tab.addEventListener('keydown', handlePhaseTabKeydown);
    });
